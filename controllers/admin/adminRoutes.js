...
router.post('/import-products', upload.single('csvFile'), async (req, res) => {
    ...
    .on('end', async () => {
      try {
        for (let product of products) {
          var newCategory = await Category.create({categoryName: product.Category});
          var newSubCategory = await SubCategory.create({
            subCategoryName: product.SubCategory,
            category_id: newCategory.id,
          });

          var newProd = await Product.create({
        ...
        res.render('import-success', { count: products.length });
      } catch (err) {
